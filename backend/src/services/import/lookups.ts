...
import { toStr, FilaExcel } from './core';
...
export const exportarLookup = async (entity: string): Promise<FilaExcel[] | null> => {
  const delegate = LOOKUP_DELEGATES[entity];
  if (!delegate) return null;
  const items = await delegate().findMany({ orderBy: { nombre: 'asc' } });
  return items.map((i: { nombre: string }) => ({ nombre: i.nombre }));
};
