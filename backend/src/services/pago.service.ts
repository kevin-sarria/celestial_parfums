...
export const createPago = (data: CreatePagoDTO, baseUrl: string) => {
  ...
  return repo.createPago(data, baseUrl);
};

export const updatePago = (id: string, data: CreatePagoDTO, baseUrl: string) => {
  if (!data.dia || !data.empresa_id || !data.valor_compra)
    throw new Error('Día, empresa y valor de compra son obligatorios');
  return repo.updatePago(id, data, baseUrl);
};
